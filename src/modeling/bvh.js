...
function selfIntersectCurve( curve ) {
	...
	for ( let i = 0; i < n; i ++ ) { // subdivide 2^n times
		...
		sub.map( c1 => {
			...
			if ( overlapBox( aabb0, aabb1 ) ) {
				...
				if ( Math.abs( s - t ) > dt ) {
					...
					if ( dis < TOL() ) { //check if two points close enough
...
function intersectCurves( c0, c1, kn = MAX_DEPTH_LEVEL, obj3d ) { // max depth (2^n devision);
...
function intersectBoxPlane( box, plane ) {
	...
	const r = extentX * Math.abs( plane.normal.x ) + extentY * Math.abs( plane.normal.y ) + extentZ * Math.abs( plane.normal.z );
...
function isEqualsBox( a, b ) {
	...
	return ( a.minX.equals( b.minX ) && a.maxX.equals( b.maxX ) ) &&
	       ( a.minY.equals( b.minY ) && a.maxY.equals( b.maxY ) ) &&
		   ( a.minZ.equals( b.minZ ) && a.maxZ.equals( b.maxZ ) );
...
function intersectSphere( sp1, sp2 ) {
	...
	const distance = Math.sqrt( ( sp1.x - sp2.x ) * ( sp1.x - sp2.x ) +
                                ( sp1.y - sp2.y ) * ( sp1.y - sp2.y ) +
