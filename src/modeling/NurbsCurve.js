...
class NurbsCurve extends Nurbs {
	...
	split( t ) {
		...
		if ( t > min && t < max ) {
			...
			const c0 = new this.constructor( this.deg, arr[ 0 ], deWeight( arr[ 1 ] ), arr[ 1 ].map( e => e.w ) );
			const c1 = new this.constructor( this.deg, arr[ 2 ], deWeight( arr[ 3 ] ), arr[ 3 ].map( e => e.w ) );
			return [ c0, c1 ];
