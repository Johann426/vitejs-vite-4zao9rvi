 ...
 *
 */
function curveSurface(C, S, t0, sor = 1) {
    //For now, no initial guess of surface, it assume that S.closestPoint() provide a good initial guess.

    const n = 128;
    const tmin = C.tmin;
    const tmax = C.tmax;
    let t = t0 ? t0 : 0.5 * (this.tmin + this.tmax);
    let min = C.getPointAt(t0)
        .sub(S.closestPoint(C.getPointAt(t0)))
        .length();
    let i, pts;
...
function toNurbs(curve) {
    //convert curve into Nurbs, if it has deg, knot, ctrl
