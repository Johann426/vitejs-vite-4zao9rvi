...
class OffsetCurve extends Parametric {
    ...
    isCusp(t) {
        // check if offset distance exceeds the radius of curvature of its progenitor curve in the concave region.
    ...
    getNurbsApproximated(obj3d) {
        ...
        for (let i = 1; i < knotmults.length - 1; i++) {
            // loop over interior knots

            const knot = knotmults[i].knot;
            const mult = knotmults[i].mult;

            if (mult == curve.deg) {
                // split at knuckle(degree-times knot multiplicities)
        ...
        for (let i = 1; i < curves.length; i++) {
            ...
            if (isFlipped) {
                ...
                if (crv0.length * crv1.length == 1) {
                    // intersection with higher resolution needed, or discontinuous case (compared with offset distance, c0 and(or) c1 are so tiny)

                    console.log(
                        "%c no intersection in concave folding",
                        "color: #ff7597; font-weight: bold; background-color: #242424"
                    );
                } else {
                    // remove tangent-flipped parts
        ...
        while (curves.length > 0) {
            ...
            if (isContinuous) {
                ...
                // case of discontinuous part

                const [crv0, crv1] = splitCurves(c0, c1, 10, obj3d); // try to split

                if (crv0.length * crv1.length == 1) {
                    // no intersection case, ignore shorter one

                    const l0 = getChordlength(c0.designPoints);
                    const l1 = getChordlength(c1.designPoints);

                    if (l0 > l1) {
                        if (l1 < Math.abs(signed_offset_distance)) {
                            // c0 maintained, c1 ignored, examine next one

                            console.log("ignore tiny length =", l1, c1);
                        } else {
                            // do nothing (otherwise intersection with higher resolution may be needed)

                            res.push(c0);
                            c0 = c1;
                        }
                    } else {
                        if (l0 < Math.abs(signed_offset_distance)) {
                            // c0 ignored, c1 maintained, examine previous one

                            console.log("ignore tiny length =", l0, c0);

                            if (res.length > 1) {
                                // do if previous one left in res

                                curves.push(c1);
                                c0 = res.pop();
                            } else {
                                // otherwise, just ignore tiny c0
        ...
        while (res.length > 1) {
            // loop over all curves (including arcs)

            c0 = res.shift();

            res.map((c1) => {
                // intesection with other curves
        ...
        curves.slice().map((curve) => {
            // check self intersection and unnecessary part
    ...
    intersectOffset(c0, c1, obj3d) {
        ...
        arr.map((r) => {
            // loop over intersection result, r = [ s, t, p, q ]

            const [s, t] = r.slice(0, 3);

            if (this.isCusp(s) || this.isCusp(t)) {
                console.log(
                    "%c no need to split in cusp",
                    "color: #ff7597; font-weight: bold; background-color: #242424"
                );
            } else {
                curves0.map((c) => {
