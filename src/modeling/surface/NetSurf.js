...
//Experimental surface averaging two loft surface
class NetSurf extends NurbsSurface {
    ...
    initialize() {
        console.warn("this class is experimental attemp");
        ...
        const knot_u_final = mergeKnots(surfaces[0].knot[0], surfaces[1].knot[0]);
        const knot_v_final = mergeKnots(surfaces[0].knot[1], surfaces[1].knot[1]);
        ...
        const ctrlp_final = p0.map((pts, j) => pts.map((p, i) => p.add(p1[j][i]).mul(0.5)));
