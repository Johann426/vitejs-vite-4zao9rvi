...
function deBoorKnots(deg: number, prm: number[]) {
    ...
    for (let i = 0; i <= deg; i++) {
        // multiplicity of degree + 1 at corner
    ...
    for (let i = 1; i < n - deg; i++) {
        // averaged over degree number of parameters
...
function knotInsert(deg: number, knot: number[], ctrl: Vector[], t: number, r = 1) {
    ...
    while (r + s > deg) {
        //Try not to have multiplicity of interior knot greater than degree
...
function knotsRemoval(deg: number, knot: number[], ctrl: Vector[], u: number, num = 1, tol = 1e-4) {
    ...
    while (num > s) {
        //Try not to remove more than multiplicity of interior knot
...
function elevateDegree(deg: number, knot: number[], ctrl: Vector[], degInc: number) {
    ...
    while (b < m) {
        ...
        for (let i = lbz; i <= ph; i++) {
            // degree elevate Bazier, only points lbz, ... , ph are used below
...
function reverseDirection(knots: number[], ctrlp: Vector[]) {
    // inpur array will be changed
...
function globalCurveInterpTngt(deg: number, prm: number[], knot: number[], pole: { point: Vector; slope: Vector }[]) {
    ...
    if (!slope.length) {
        ...
        // if directional constraint(s) exist
        ...
        for (let i = 0; i < n; i++) {
            ...
            if (hasValue) {
                // additional ctrlp for directional constraint
