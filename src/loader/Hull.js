...
class Hull {
	...
	drawCons( buffer ) {
		...
		curve.add( new Vector( 0.00 * this.LBP, 4.0, 0 ) );
		curve.add( new Vector( 0.25 * this.LBP, 2.8, 0 ) );
		curve.add( new Vector( 0.50 * this.LBP, 2.8, 0 ) );
		curve.add( new Vector( 0.75 * this.LBP, 2.8, 0 ) );
		curve.add( new Vector( 1.00 * this.LBP, 4.0, 0 ) );
	...
	dmpRead( txt ) {
		...
		while ( arr[ i ] != undefined ) {
			...
			if ( isNew ) { // trying to find point info
			...
			if ( isPts ) {
				...
				if ( row.length == 5 ) {
					...
					for ( let j = 0; j < n; j ++ ) {
						...
						if ( isSame ) { // trying to avoid duplicate point
		...
		for ( let i = 0; i < this.curves.length; i ++ ) {
			...
			switch ( name.slice( 0, 3 ) ) {
				...
				case 'WAT' :
					...
					if ( pts.length > 1 ) { // preliminary treatment
...
function lazyNormal( curve ) {
	...
	return new Plane().setFromNormalAndCoplanarPoint( new Vector( round( n.x, 9 ), round( n.y, 9 ), round( n.z, 9 ) ).normalize(), p0 );
