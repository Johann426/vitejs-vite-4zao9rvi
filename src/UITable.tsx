...
type Cell = {
  ...
  onBlur: () => void;
  onKeyDown: (i: number, j: number, e: React.KeyboardEvent<HTMLInputElement>) => void;
  onPointerDown: () => void;
  onPointerEnter: () => void;
};

function Td({ i, j, val, bool, className, onDoubleClick, onChange, onBlur, onKeyDown, onPointerDown, onPointerEnter }: Cell) {
  return (
    <td className={className} onDoubleClick={onDoubleClick} onPointerDown={onPointerDown} onPointerEnter={onPointerEnter} onClick={(e) => e.currentTarget.focus()}>
...
export default function Table() {
  ...
  const onKeyDown = (i: number, j: number, e: React.KeyboardEvent<HTMLInputElement>): void => {
    switch (e.key) {
      case "Enter":
      case "ArrowDown":
